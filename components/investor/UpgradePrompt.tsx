  ...
  return (
    <div className="flex min-h-[420px] flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 bg-gray-50/80 px-6 py-12 text-center">
      ...
      <h2 className="mt-5 text-xl font-bold text-gray-900">
        {title ?? `Unlock with ${required.label}`}
      </h2>
      <p className="mt-2 max-w-md text-sm text-gray-600">
        {description ??
          `This feature requires ${required.label} or higher. Upgrade your investor tier to access ${
            feature?.replace(/_/g, ' ') ?? 'this module'
          }.`}
      </p>
      ...
      <p className="mt-3 text-xs text-gray-500">
        {required.badge} Min. investment {required.minimumInvestment.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })} · ROI {required.roiRange}
      </p>
      <Link
        href="/invest"
      ...
      >
        ...
        {next ? `Upgrade to ${getInvestorTierConfig(next).label}` : 'View Investment Plans'}
      </Link>
    </div>
  )
}
