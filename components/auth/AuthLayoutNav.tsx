  ...
  return (
    <header className="absolute inset-x-0 top-0 z-10 bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-2 px-4 sm:gap-3 sm:px-6">
        <Link
          href="/"
          className="inline-flex min-w-0 shrink items-center gap-2 rounded-lg px-1 py-1 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
        >
          <ArrowLeft className="size-4 shrink-0" aria-hidden />
          <span className="truncate sm:hidden">{tNav('home')}</span>
          <span className="hidden truncate sm:inline">{t('backToHome')}</span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          {authToggle ? (
            <Link
              ...
              className="rounded-lg px-2 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-primary/10 hover:underline sm:px-3 sm:text-sm"
            >
              {authToggle.label}
            </Link>
          ) : null}
